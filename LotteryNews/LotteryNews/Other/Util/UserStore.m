...
- (void)getCategories:(isRequestFinsh)isFinsh{
    NSString *sever = UserDefaultObjectForKey(LOTTERY_HOST_NAME_SEVER);
    NSString *url = [NSString stringWithFormat:@"%@/lottery/categories",sever];
...
- (void)requestPlayTypeDefult:(isRequestFinsh)isFinsh{
...
- (void)getCategoriesAndPlayTypeDefult:(isRequestFinsh)isFinsh{
  __block  BOOL OK1,OK2,OK3 = NO;
    [self checkReviewTheStatus:^(BOOL isFinsh) {
        OK1 = isFinsh;
    }];
    [self getCategories:^(BOOL isFinsh) {
        OK2 = isFinsh;
    }];
    [self requestPlayTypeDefult:^(BOOL isFinsh) {
        OK3 = isFinsh;
    }];
    if (OK3&&OK2&&OK1) {
        isFinsh(YES);
...
#pragma mark-- 内购接口状态
- (void)checkReviewTheStatus:(isRequestFinsh)isFinsh{
...
- (void)getRoadBeadAnalysis:(NSInteger)type SucessBlock:(sucessCompleteBlock)sucesBlock failureBlock:(failureCompleteBlock)failureBlock{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/api/gdkl10/%@",StockMarket_HOST_NAME,statisticalType];
    [[HRNetworkTools sharedNetworkTools]GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        sucesBlock(task,responseObject);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        failureBlock(task,error);
...
- (void)getNumbertrenddata:(NSInteger)ball SucessBlock:(sucessCompleteBlock)sucesBlock failureBlock:(failureCompleteBlock)failureBlock{
    NSString *ballNumber = [NSString stringWithFormat:@"%ld",(long)ball];
    NSString *url = [NSString stringWithFormat:@"%@/api/gdkl10/numbertrenddata?ball=%@",StockMarket_HOST_NAME,ballNumber];
    [[HRNetworkTools sharedNetworkTools]GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        sucesBlock(task,responseObject);
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        failureBlock(task,error);
...
-(void)getHistorySucessBlock:(sucessCompleteBlock)sucesBlock failureBlock:(failureCompleteBlock)failureBlock{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/api/gdkl10/history?dateData=%@",StockMarket_HOST_NAME,current];
    [[HRNetworkTools sharedNetworkTools]GET:url parameters:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if (!error||[error isEqualToString:@""]) {
            ...
            for (NSArray *items in itemArray) {
                MoreHistoryModel *model = [[MoreHistoryModel alloc]init];
